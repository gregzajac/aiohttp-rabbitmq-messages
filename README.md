...
1. Server A (front-server) with REST API managing two endpoints
  ...
  --------  |  ------  |  -----  |  ------  |  -------
  `http://localhost:8080/api`  |  `POST`  |  `JSON {key:value}, key(str), value(int/float)`  |  `JSON {"success":boolean, "data"/"message": string}`  |  `curl -i -X POST -H "Content-Type: application/json" -d '{"age":25}' http://localhost:8080/api`
  `http://localhost:8080/api`  |  `GET`  |  `attribute key in URL`  |  `JSON {"success":boolean, "data"/"message": string}`  |  `curl -i http://localhost:8080/api?key=age`
