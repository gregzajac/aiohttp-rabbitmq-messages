...
async def on_set_value_message(message):
    ...
    await save_message(
        app.new_app.db_connection, message_key, message_value
    )
...
async def on_get_value_message(exchange, message):
    with message.process():
        ...
        message_value = await get_message_value(
            app.new_app.db_connection, message_key
        )

        print(f' [x] Publish callback for message key: {message_key}')
        await exchange.publish(
            Message(
                body=str(message_value).encode(),
...
async def handle_get_value():
    ...
    await queue.consume(
        partial(on_get_value_message, channel.default_exchange)
    )